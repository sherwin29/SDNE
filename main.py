        ...
        while (True):
            ...
            if graph_data.is_epoch_end:
                ...
                if epochs % config.display == 0:
                    ...
                    file_name = config.embedding_filename  \
                                + '_m_' + str(config.struct[1]) \
                                + '_a_' + str(config.alpha) \
                                + '_r_' + str(config.reg) \
                                + '_b_' + str(config.beta) \
                                + '_dbnlr_' + str(config.dbn_learning_rate) \
                                + '_dbnepo_' + str(config.dbn_epochs) \
                                + '_lr_' + str(config.learning_rate) \
                                + '_epo_' + str(epochs)
                                
                    sio.savemat(file_name + '_embedding.mat', {'embedding':embedding})
